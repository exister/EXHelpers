...
#ifndef LOCAL
    [[BITHockeyManager sharedHockeyManager] configureWithBetaIdentifier:kEXHockeyAppApiKeyBeta liveIdentifier:kEXHockeyAppApiKeyLive delegate:self];
...
{
    DDLogInfo(@"%@", THIS_METHOD);
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onDeviceRegistered:) name:kEXDeviceRegisteredNotification object:nil];
...
- (void)initAuth
{
    [EXRegistrationHelper setDeviceRegistered:NO];

    //load auth token from keychain to cache
    [EXRegistrationHelper authToken];

    //generate device id
    NSString *deviceId = [EXRegistrationHelper deviceId];
    if (deviceId == nil) {
        [EXRegistrationHelper setDeviceId:[NSString stringWithUUID]];
    }
    DDLogInfo(@"deviceId: %@", deviceId);

    [[EXRegistrationHelper sharedInstance] startObserving];
}
...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken
{
    ...
    [EXRegistrationHelper setPushToken:pushToken];

    [[NSNotificationCenter defaultCenter] postNotificationName:kEXRegisteredForRemoteNotifications object:nil];
...
- (void)application:(UIApplication *)application didFailToRegisterForRemoteNotificationsWithError:(NSError *)error
{
    ...
    if (![EXRegistrationHelper pushToken]) {
        [EXRegistrationHelper setPushToken:[NSString stringWithUUID]];
    }
#endif
}
...
- (void)onDeviceRegistered:(NSNotification *)notification {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kEXDeviceRegisteredNotification object:nil];
}

- (void)registerPushId {
    if (![[EXRegistrationHelper pushToken] stringIsEmpty]) {
        [[EXRestAPI sharedInstance] registerPushToken:[EXRegistrationHelper pushToken] success:^(AFHTTPRequestOperation *operation, id o) {
...
{
    DDLogInfo(@"%@", THIS_METHOD);
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kEXSplashScreenJobDoneNotification object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kEXSplashScreenAllJobsDoneNotification object:nil];
    ...
    [UIView animateWithDuration:1.0
                     ...
                     completion:^(BOOL finished){
                         ...
                         [[NSNotificationCenter defaultCenter] postNotificationName:kEXSplashScreenFinished object:nil];
                     }];
}
...
- (void)runSplashScreenJobs
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onSplashScreenJobDone:) name:kEXSplashScreenJobDoneNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onSplashScreenAllJobsDone:) name:kEXSplashScreenAllJobsDoneNotification object:nil];

    //register device
    [[EXRestAPI sharedInstance] registerDeviceWithSuccessBlock:^(AFHTTPRequestOperation *operation, id responseObject)
    {
        DDLogInfo(@"Register device finished");
        [EXRegistrationHelper setDeviceRegistered:YES];
        [[NSNotificationCenter defaultCenter] postNotificationName:kEXDeviceRegisteredNotification object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:kEXSplashScreenJobDoneNotification object:nil];
    }
    failure:^(AFHTTPRequestOperation *operation, NSError *error)
    {
...
- (void)onSplashScreenJobDone:(NSNotification *)notification
{
    ...
    if (self.splashScreenJobsNumber <= 0) {
        [[NSNotificationCenter defaultCenter] postNotificationName:kEXSplashScreenAllJobsDoneNotification object:nil];
    }
}
